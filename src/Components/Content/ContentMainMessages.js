...
const ContentMainMessages = () => {
    ...
    const chatId = useSelector((state) => state.chatReducer.id);
    ...
    useEffect(() => {
        if (chatId !== 0) {
            const response = axios.get(API_PATHS.GET_CHAT_MESSAGES(chatId))
                .then((response) => {
        ...
        <>
            {
                ...
                <ContentMainMessagesStyled chatId={chatId === 0}>
                    {
                        chatId !== 0
                            ?
                            messages.map((value, index) => {
                                return (
                                    <ContentMainMessage
                                        key={value[0]} side={(userId === value[2]) ? 0 : 1}
                                        message={value[4]} time={formatAMPM(new Date(value[5]))}
                                    />
                                )
                            })
                            :
                            <div>Chat seçilmedi</div>
                    }
